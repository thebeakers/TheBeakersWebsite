<!-- This file is an EXACT COPY of src/routes/biology/+page.svelte -->
<script lang="ts">
	...
	// For Earth Science, the pageTitle might be "Earth Science" but categoryName "earth"
	// We need to ensure isEmptyFeatured check uses the slug-like version from categoryName
	// This assumes categoryName used in +page.server.ts for emptyFeatured.id is 'earth'
	const categorySlugForEmptyCheck =
		data.pageTitle === 'Earth Science' ? 'earth' : data.pageTitle.toLowerCase();
	const isEmptyFeatured = data.featuredEssay?.id === `empty-featured-${categorySlugForEmptyCheck}`;
</script>
